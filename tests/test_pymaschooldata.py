...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ma.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pymaschooldata as ma
        assert hasattr(ma, 'fetch_enr')
        assert hasattr(ma, 'fetch_enr_multi')
        assert hasattr(ma, 'tidy_enr')
        assert hasattr(ma, 'get_available_years')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ma.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = ma.fetch_enr(years['max_year'])
        # Total enrollment in Massachusetts varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Massachusetts should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ma.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ma.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pymaschooldata as ma
        years = get_test_years()
        df_single = ma.fetch_enr(years['max_year'])
        df_multi = ma.fetch_enr_multi([years['max_year']])
