# ==============================================================================
# LIVE Pipeline Tests for maschooldata
# ==============================================================================
#
...
test_that("Massachusetts DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Massachusetts DOE URL and verify HTTP 200")
})
...
test_that("Can download Massachusetts enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Massachusetts enrollment file", {
  skip_if_offline()
...
test_that("maschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- maschooldata::get_available_years()
  ...
  tryCatch({
    raw <- maschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- maschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- maschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- maschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- maschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- maschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- maschooldata:::get_cache_path(2024, "enrollment")
