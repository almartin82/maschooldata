...
# Available years: 1994-2025
#
# ==============================================================================

#' Fetch Massachusetts enrollment data
#'
#' Downloads and processes enrollment data from the Massachusetts Department
...
fetch_enr <- function(end_year, tidy = TRUE, use_cache = TRUE) {
  ...
  raw <- get_raw_enr_api(end_year)

  # Process to standard schema
  processed <- process_enr_api(raw, end_year)
  ...
  results <- purrr::map(
    ...
    function(yr) {
      ...
      tryCatch(
        ...
        error = function(e) {
          warning(
            "Failed to fetch enrollment data for ", yr, ": ", e$message,
